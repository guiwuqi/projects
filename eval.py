...
def plot_results(cfg: Config, train_rewards: List[float], eval_data: Dict[str, np.ndarray]) -> None:
    ...
    plt.figure(figsize=(8, 4.5))
    plt.step(eval_data["times"], eval_data["active_counts"], where="post", lw=1.8)
    plt.xlabel("Time (s)")
    plt.ylabel("# Active Ports")
    plt.title("Active Port Count vs Time")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(fig_dir, "active_ports_vs_time.svg"), dpi=200)
    plt.close()
...
if __name__ == "__main__":
    ...
    print(f"Avg active ports: {eval_data['active_counts'].mean():.4f}")
