...
def get_ula_capacity(cfg, n_0, clusters, uav_positions):
    ...
    for t in range(cfg.T):
        tx_ref = uav_positions[t]  # UAV中心位置
        ...
        for cl in clusters:
            ...
            betas = cl["betas"]
            gains = cl["gains"]
            scatterer_pos = cl["scatterer_positions"]  # (P, 3)

            # 计算实际距离（与FAS一致）
            tx_to_scatterer = scatterer_pos - tx_ref[None, :]  # (P, 3)
...
def run_comparison():
    ...
    ula_caps = get_ula_capacity(cfg, ula_port_count, clusters, uav_positions)

    # 确保长度一致
    if len(ula_caps) < len(fas_caps):
        ula_caps = np.append(ula_caps, ula_caps[-1])

    # --- 3. 验证：打印几个时间点的ULA容量，看看是否有变化 ---
    print("\nULA Capacity at different time points:")
    for t in range(0, min(5, len(ula_caps))):
        print(f"  t={t}: {ula_caps[t]:.4f} bps/Hz")
    ...
    plt.plot(times, fas_caps, 'r-o', label=f'FAS (Memetic PPO, K={cfg.K})', markersize=4)
    ...
    # 新增：绘制容量差值
    # plt.figure(figsize=(10, 5))
    # capacity_diff = fas_caps - ula_caps[:len(fas_caps)]
    # plt.plot(times, capacity_diff, 'g-^', label='FAS - ULA Capacity Gain', markersize=4)
    # plt.axhline(y=0, color='r', linestyle='--', alpha=0.5)
    # plt.xlabel('Time (s)')
    # plt.ylabel('Capacity Gain (bps/Hz)')
    #plt.title('FAS Capacity Gain Over ULA')
    # plt.legend()
    # plt.grid(True, alpha=0.3)
    # plt.tight_layout()
    #plt.savefig("comparison_plots/capacity_gain.svg", dpi=300)
