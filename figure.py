...
def get_ula_capacity(cfg, n_0, clusters, uav_positions):
    ...
    for t in range(cfg.T):
        ...
        for cl in clusters:
            ...
            betas = cl["betas"]
            gains = cl["gains"]
            directions = build_path_directions(alphas, betas)

            rx_phase = np.exp(-1j * k * (rx_pos @ directions.T))
            tx_phase = np.exp(-1j * k * (tx_pos_t @ directions.T))
            doppler = doppler_phase(cfg, directions, time_s, k)
...
def run_comparison():
    ...
    plt.plot(times, ula_caps, 'b--s', label=f'ULA (Fixed Antennas = {ula_port_count})')
