...
class FASEnv(gym.Env):
    ...
    def _get_state(self) -> np.ndarray:
        ...
        # 容量空间梯度：对每个激活端口估计右邻居容量-左邻居容量，并取均值
        ...
        # 信道相位空间变化率：端口维度相邻相位差均值（反映多径干涉剧烈程度）
        phase_variation = 0.0
        ...
        return np.concatenate(
            [
                ...
                np.array([capacity_gradient], dtype=np.float32),  # 1维容量梯度（新增）
                np.array([phase_variation], dtype=np.float32),  # 1维相位变化率（新增）
                mask,  # N维端口状态
    ...
    def step(self, action: int):
        ...
        # --- [关键修改 2: 计算不采取动作的基准容量 (No-Op Baseline)] ---
        idx_old = np.array(sorted(self.active_set), dtype=int)
        # 计算：如果这一步什么都不做，在当前信道下的容量
        ...
        # 1. action_gain: 相比于“不动作”带来的纯增益（这是我们奖励的核心）
        action_gain = self.current_capacity - cap_no_action

        # 2. reward_gain: 相比于“上一个时间步”的容量变化（用于 info 监控）
        reward_gain = self.current_capacity - old_capacity

        # 3. baseline_contrast: 相比于 ULA 基准的差值
        # 注意：确保你的类里有 self.ula_baseline_capacity，如果没有，可以用 self.cfg.snr_db 相关的估算值
        baseline_contrast = self.current_capacity - getattr(self, 'ula_baseline_capacity', 0.0)

        # 4. advantage_reward: 优势奖励
        advantage_reward = max(0.0, baseline_contrast)

        # --- [最终奖励计算] ---
        reward = action_gain * 10.0 + (self.current_capacity / 5.0)
        ...
        info = {
            ...
            "reward_terms": {
                ...
                "advantage_reward": float(advantage_reward),
