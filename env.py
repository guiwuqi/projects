...
class FASEnv(gym.Env):
    ...
    def _sample_with_spacing_constraint(self, candidate_pool: np.ndarray, target_k: int) -> Set[int]:
        ...
        "请减小 K 或减小 min_port_spacing_in_lambda。"
        )
    ...
    def _compute_realtime_channel(self) -> np.ndarray:
        ...
        rx_ref = np.mean(self.rx_port_positions, axis=0)  # RX中心
        rx_offsets = self.rx_port_positions - rx_ref[None, :]
        ...
        for cl in self.clusters:
            ...
            betas = cl["betas"]
            gains = cl["gains"]
            scatterer_pos = cl["scatterer_positions"]  # (P, 3) 获取散射体位置

            # 使用实际距离计算（与channel.py一致）
            # UAV到散射体距离
            tx_to_scatterer = scatterer_pos - uav_pos_t[None, :]  # (P, 3)
            dist_tx = np.linalg.norm(tx_to_scatterer, axis=1)  # (P,)

            # 散射体到RX中心距离
            rx_to_scatterer = scatterer_pos - rx_ref[None, :]  # (P, 3)
            dist_rx = np.linalg.norm(rx_to_scatterer, axis=1)  # (P,)

            # 正确的距离相位项
            distance_phase = np.exp(-1j * k * (dist_tx + dist_rx))  # (P,)
    ...
    def step(self, action: int):
        ...
        # 训练时执行 policy 原始动作，避免 (s, a*, r) 丢失 credit assignment
        ...
        # 奖励函数：容量增益 + 正增益奖励 + 动作对比度奖励
        gain = self.current_capacity - old_capacity
        gain_term = gain / (abs(old_capacity) + 1e-6)
        positive_bonus = max(0.0, gain)

        # 估计当前状态下动作容量分布，提升区分度
        contrast_term = 0.0
        if len(legal) > 1:
            sample_size = min(len(legal), max(2, int(self.cfg.reward_contrast_sample_k)))
            sampled_actions = self.rng.choice(np.array(legal, dtype=int), size=sample_size, replace=False)
            sampled_caps = np.array([self._capacity_if_apply_action(int(a)) for a in sampled_actions], dtype=float)
            cap_std = float(np.std(sampled_caps))
            if cap_std > 1e-8:
                contrast_term = (self.current_capacity - float(np.mean(sampled_caps))) / cap_std

        reward = (
            self.cfg.reward_gain_weight * gain_term
            + self.cfg.reward_positive_gain_bonus * positive_bonus
            + self.cfg.reward_contrast_weight * contrast_term
        )
        ...
        info = {
            ...
            "reward_terms": {
                ...
                "positive_bonus": float(positive_bonus),
                "contrast_term": float(contrast_term),
            },
        }
