...
class FASEnv(gym.Env):
    ...
    def _compute_realtime_channel(self) -> np.ndarray:
        ...
        rx_ref = np.mean(self.rx_port_positions, axis=0)
        ...
        for cl in self.clusters:
            ...
            # 距离相关相位项: exp(-j*2π/λ * (ξ_T,ln(t)+ξ_R,ln))
            xi_t = directions @ tx_ref  # (P,)
            xi_r = directions @ rx_ref  # (P,)
            distance_phase = np.exp(-1j * k * (xi_t + xi_r))  # (P,)
