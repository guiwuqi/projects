...
class FASEnv(gym.Env):
    ...
    def __init__(self, cfg: Config):
        ...
        self.max_jump_radius = 5  # 添加最大跳变半径控制
    ...
    def _build_action_catalog(self) -> List[Tuple[str, int, int]]:
        actions: List[Tuple[str, int, int]] = [("none", -1, 0), ("none", -1, 0)]
        # 添加无操作动作
    ...
    def _action_neighbors(self, action_idx: int) -> List[int]:
        ...
        for dn in [-1, 1]:
            ...
            if 0 <= nn < self.cfg.N:
                if op == "add":
                    neighbors.add(nn)  # add block [0, N)
                elif op == "remove":
                    neighbors.add(self.cfg.N + nn)  # remove block [N, 2N)
                else:
                    # replace block starts at 2N
                    shift_id = self.replace_shifts.index(shift)
                    block = 2 * self.cfg.N + nn * len(self.replace_shifts) + shift_id
                    neighbors.add(block)
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        window = np.arange(max(0, center - 20), min(self.cfg.N, center + 21))
        if len(window) < init_k:
            window = np.arange(self.cfg.N)
        selected = self.rng.choice(window, size=init_k, replace=False)
        self.active_set = set(int(x) for x in selected)
    ...
    def step(self, action: int):
        ...
        # 计算基础奖励（使用新配置的容量）
        reward = self.current_capacity / len(self.active_set)

        # 如果端口配置有变化，检查容量是否增加
        if old_config != self.active_set:
            # 计算容量增加量
            reward *= 1.05
            capacity_increase = self.current_capacity - old_capacity
            # 只有当容量确实增加时才给予额外奖励
            if capacity_increase >= 0:
                # 额外奖励与容量增加量成正比，但不超过最大额外奖励
                # extra_reward = min(0.1, capacity_increase * 2.0)  # 系数可调整
                reward += 10 * capacity_increase
