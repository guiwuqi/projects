...
def reward_function(C, delta_error, reward_mode="combined"):
    """奖励函数：支持容量/误差/联合三种模式。"""
    # 标准化容量奖励
    capacity_norm = 0.1 * C
    ...
    if reward_mode == "capacity":
        total_reward = capacity_norm
    elif reward_mode == "error":
...
class FASA2GEnv(gym.Env):
    def __init__(self, use_local_search=False, search_radius=1, reward_mode="combined"):
        super(FASA2GEnv, self).__init__()
        self.use_local_search = use_local_search
        self.search_radius = search_radius
        if reward_mode not in {"combined", "capacity", "error"}:
            raise ValueError("reward_mode must be one of: combined, capacity, error")
        ...
        # 修改状态空间 - 移除角度变量
        # 根据理论计算设置精确边界
        delta_d_min = 2.0 / (Q - 1)  # 当W=2时
        delta_d_max = 20.0 / (Q - 1)  # 当W=20时
        self.observation_space = spaces.Dict({
            'W': spaces.Box(low=2, high=20.0, shape=(1,), dtype=np.float32),
            'Q_sub': spaces.Box(low=1, high=30, shape=(1,), dtype=np.int32),
            'delta_d': spaces.Box(low=delta_d_min, high=delta_d_max, shape=(1,), dtype=np.float32),
            'SNR': spaces.Box(low=0, high=30, shape=(1,), dtype=np.float32)
        })
    ...
    def _calculate_ULA_baseline(self, Q_sub):
        ...
        P = self.P
        t = 2.0
        delta_R = self.lambda_wave / 2  # ULA端口间距固定为半波长
        ...
        power = np.mean(np.abs(H_ULA) ** 2)
        if power > 0:
            H_ULA = H_ULA / np.sqrt(power)
        return H_ULA
    ...
    def _apply_action(self, action):
        ...
        new_W = np.clip(self.state['W'][0] + w_change, 2, 20.0)
        self.state['W'] = np.array([new_W], dtype=np.float32)

        # 更新端口间距
        normalized_spacing = new_W / (Q - 1) #以波长倍数为单位
    ...
    def _calculate_reward(self, C, delta_error):
        """计算奖励，现在包含模型误差"""
        return reward_function(C, delta_error, reward_mode=self.reward_mode)
    ...
    def _calculate_NLoS_component(self):
        ...
        for p in range(P):
            ...
            for q in range(Q_sub):
                # FAS端口位置（使用固定角度）
                k_q = (Q_sub - 2 * q + 1) / 2
                d_q = k_q * self.state['delta_d'][0] * self.lambda_wave * np.array([
                    np.cos(self.theta_R_fixed) * np.cos(self.psi_R_fixed),  # 使用固定值
                    np.cos(self.theta_R_fixed) * np.sin(self.psi_R_fixed),  # 使用固定值
                    np.sin(self.theta_R_fixed)  # 使用固定值
                ]) - np.array([self.D0, 0, 0])
                ...
                for l_idx, cluster in enumerate(self.scatterers):
                    for n_idx, scatterer in enumerate(cluster):
                        ...
                        # 3. FAS端口阵列相位（使用固定角度）
                        phase3 = 2 * np.pi * k_q * self.state['delta_d'][0] * (
                                np.cos(alpha_R) * np.cos(beta_R) * np.cos(self.theta_R_fixed) +  # 使用固定值
                                np.sin(beta_R) * np.sin(self.theta_R_fixed)  # 使用固定值
                        )
                        # 4. UAV运动相位
