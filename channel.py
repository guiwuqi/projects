...
def sample_paths(cfg: Config, rng: np.random.Generator) -> List[Dict[str, np.ndarray]]:
    ...
    # 定义散射体分布的平均距离
    mean_scatterer_distance = 100.0  # 单位：米，根据场景调整
    ...
    for mu_az_deg, mu_el_deg in zip(cfg.cluster_mean_aoa_deg, cfg.cluster_mean_eoa_deg):
        ...
        scatterer_positions = np.stack([
            distances * np.cos(betas) * np.cos(alphas),
            distances * np.cos(betas) * np.sin(alphas),
            distances * np.sin(betas),
        ], axis=1)  # (Npath, 3)
        ...
        clusters.append({
            ...
            "betas": betas,
            "gains": gains,
            "scatterer_positions": scatterer_positions  # 新增：散射体位置
        })
